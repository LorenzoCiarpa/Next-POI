...
from utils.func_top_k import get_top_k_users

with open('users_clustered.pkl', 'rb') as f:
    users_clustered = pickle.load(f)

class hmt_grn(nn.Module):

    def __init__(self, arg, top_k = 5):
        ...
        self.fuseDenseClusters = nn.Linear(arg['userEmbed_dim']*3 + arg['hidden_dim'],
                                   arg['num_classes'] * 1, bias=True)
        
        self.fc_top_k = nn.Linear(3360, arg['userEmbed_dim'], bias=True)
    ...
    def forward(self, input, mode, arg):
        ...
        if mode == 'train':
            ...
            user_cluster_embeds = t.stack([t.stack(sublist) for sublist in user_cluster_embeds])


            # print(f"user_cluster_embeds: {len(user_cluster_embeds)}")
            # print(f"user_cluster_embeds: {len(user_cluster_embeds[0])}")
            # print(f"user_cluster_embeds: {user_cluster_embeds[0][0].shape}")
            # print(f"user_cluster_embeds: {user_cluster_embeds.shape}")
            ...
            user_cluster_embeds = user_cluster_embeds.unsqueeze(0)
        ...
        # print(f"userEmbedSeq: {userEmbedSeq.shape}")
        # print(f"rnn_out: {rnn_out.shape}")
        # print(f"clustersEmbedSeq: {clustersEmbedSeq.shape}")

        user_cluster_embeds = user_cluster_embeds.float()
        userClustersEmbedSeq = self.dropout(self.fc_top_k(user_cluster_embeds))#(batch_size, time_steps, embed_dim)

        
        # print(f"userClustersEmbedSeq: {userClustersEmbedSeq.shape}")

        finalEmbed = t.cat((rnn_out, userEmbedSeq, clustersEmbedSeq, userClustersEmbedSeq), dim=2) #(batch_size, time_steps, hidden_size+embed_dim)
