...
def gmm_clustering(data, k, max_iters=100, tol=1e-4):

    """
    Esegue il clustering utilizzando il Gaussian Mixture Model sui dati.
    ...
    - labels: array di etichette per ogni punto
    """
    # Convertiamo la lista di array in un array 2D se necessario
    # data = np.array(data)
    
    # Eseguiamo il Gaussian Mixture Model
    gmm = GaussianMixture(n_components=k, max_iter=max_iters, tol=tol, random_state=seed)
    print("fitting gmm...")
    gmm.fit(data)
    
    # Otteniamo le etichette e i centroidi
...
if __name__ == '__main__': 
    ...
    for dataSource in ['gowalla']:
        ...
        # centroids, labels = gmm_clustering(array_2d, k=20, max_iters=20)
        ...
        # with open('clusters_gmm.pkl', 'wb') as f:
        #     pickle.dump(clusters, f)
        #     print("saved pickle file")

        with open('clusters_gmm.pkl', 'rb') as f:
            clusters = pickle.load(f)
        ...
        for idx, i in enumerate(labels):
            ...
            if i not in labels_count:
                ...
                users_clustered[i] = []
            labels_count[i] += 1
            users_clustered[i].append(array_2d[idx])
        print(f"labels_count: {labels_count}")
        print(f"labels: {labels}")

        # with open('users_clustered.pkl', 'wb') as f:
        #     pickle.dump(users_clustered, f)
        #     print("saved pickle file")
        ...
        print(np.array(users_clustered[12]).shape)