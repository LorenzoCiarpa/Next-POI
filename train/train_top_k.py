...
from ..models.model_top_k import *
import random
import numpy as np
    ...
    for dataSource in ['gowalla']:
        ...
        for epoch in range(1, arg['epoch'] + 1):
            ...
            for x, user, y in classification_pbar:
                ...
                # print(f"nextgeohashPred_2.shape: {nextgeohashPred_2.shape}")
                
                truth = y.to(t.int64).cuda()
            ...
            if avgLossDict['Next POI Classification'] < best_loss:
                best_loss = avgLossDict['Next POI Classification']
                torch.save(classification.state_dict(), f"checkpoint/GRN_cluster_{epoch}.pth")
